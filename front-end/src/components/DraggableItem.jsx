...
export default  function DraggableItem(props) {
    ...
    const displayItemLabels = props.displayItemLabels;
    ...
    return (
        ...
        {displayItemLabels && (
            <div className="item-label"> {isEditing ? (
                    <input className="item-label-input" value={label} onChange={(e) => setLabel(e.target.value)} onBlur={() => props.updateLabel?.(props.id, label)}
                        onKeyDown={(e) => {
                            if (e.key === "Enter") {
